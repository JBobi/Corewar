...
{
    //mvprintw(0, 130, "player :  %s", player->name);
    //mvprintw(1, 130, "command: %s", op_tab[corewar->memory[get_mod(player->pc, MEM_SIZE)] - 1].mnemonique);
    int command = corewar->memory[get_mod(player->pc, MEM_SIZE)];
    ...
    ////    mvprintw(i + 2, 130, "args[%d] :  %d", i, args[i]);
    //refresh();
    //usleep(1000);
