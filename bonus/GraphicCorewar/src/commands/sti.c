...
void print_bytes(int depart, corewar_t *corewar, int bytes_nbr)
{
    ...
    for (int i = get_mod(depart, MEM_SIZE); u < bytes_nbr; i++, u++) {
        attron(COLOR_PAIR((corewar->color[get_mod(i, MEM_SIZE)] == 0) ? 8 : (int)corewar->color[get_mod(i, MEM_SIZE)]));
        mvprintw(i / 128, i % 128, "%c", corewar->color[get_mod(i, MEM_SIZE)] + '0');
        attroff(COLOR_PAIR((corewar->color[get_mod(i, MEM_SIZE)] == 0) ? 8 : (int)corewar->color[get_mod(i, MEM_SIZE)]));
    }
    refresh();
}
